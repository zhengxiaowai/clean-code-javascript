...
* [简介](#简介)
* [**变量**](#变量)
    * [使用有意义并且可读的变量名称](#使用有意义并且可读的变量名称)
    * [为相同类型的变量使用相同的词汇](#为相同类型的变量使用相同的词汇)
    * [使用可搜索的名称](#使用可搜索的名称)
    * [使用解释性的变量](#使用解释性的变量)
    * [避免心理映射](#避免心理映射)
    * [不添加不必要的上下文](#不添加不必要的上下文)
    * [使用默认变量替代短路运算或条件](#使用默认变量替代短路运算或条件)
* [**函数**](#函数)
    * [函数参数 (两个以下最理想)](#函数参数-两个以下最理想)
    * [函数应当只做一件事情](#函数应当只做一件事情)
    * [函数名称应该说明它要做什么](#函数名称应该说明它要做什么)
    * [函数应该只有一个抽象级别](#函数应该只有一个抽象级别)
    * [移除冗余代码](#移除冗余代码)
    * [使用 Object.assign 设置默认对象](#使用-objectassign-设置默认对象)
    * [不要使用标记位做为函数参数](#不要使用标记位做为函数参数)
    * [避免副作用](#避免副作用)
    * [不要写入全局函数](#不要写入全局函数)
    * [函数式编程优于指令式编程](#函数式编程优于指令式编程)
    * [封装条件语句](#封装条件语句)
    * [避免负面条件](#避免负面条件)
    * [避免条件语句](#避免条件语句)
    ...
    * [移除僵尸代码](#移除僵尸代码)
* [**对象和数据结构**](#对象和数据结构)
    * [使用 getters 和 setters](#使用-getters-和-setters)
    * [让对象拥有私有成员](#让对象拥有私有成员)
* [**类**](#类)
    * [ES2015/ES6 类优先与 ES5 纯函数](#es2015es6-类优先与-es5-纯函数)
    * [使用方法链](#使用方法链)
    * [组合优先于继承](#组合优先于继承)
* [**SOLID**](#solid)
    * [单一职责原则 (SRP)](#单一职责原则-srp)
    * [开闭原则 (OCP)](#开闭原则-ocp)
    * [里氏代换原则 (LSP)](#里氏代换原则-lsp)
    * [接口隔离原则 (ISP)](#接口隔离原则-isp)
    * [依赖反转原则 (DIP)](#依赖反转原则-dip)
* [**测试**](#测试)
    * [一个测试一个概念](#一个测试一个概念)
* [**并发**](#并发)
    * [使用 Promises, 不要使用回调](#使用-promises-不要使用回调)
    * [Async/Await 比 Promises 更加简洁](#asyncawait-比-promises-更加简洁)
* [**错误处理**](#错误处理)
    * [不要忽略捕捉到的错误](#不要忽略捕捉到的错误)
    * [不要忽略被拒绝的 promise](#不要忽略被拒绝的-promise)
* [**格式化**](#格式化)
    * [使用一致的大小写](#使用一致的大小写)
    * [函数的调用方与被调用方应该靠近](#函数的调用方与被调用方应该靠近)
* [**注释**](#注释)
    * [仅仅对包含复杂业务逻辑的东西进行评论](#仅仅对包含复杂业务逻辑的东西进行评论)
    * [不要在代码库中保存注释掉的代码](#不要在代码库中保存注释掉的代码)
    * [不要有日志式的评论](#不要有日志式的评论)
    * [避免占位符](#避免占位符)
